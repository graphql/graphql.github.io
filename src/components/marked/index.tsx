...
InlineLexer.prototype.output = function (src) {
  ...
  while (src) {
    ...
    if ((cap = this.rules.autolink.exec(src))) {
      ...
      out.push(
        createElement("a", { href: this.sanitizeUrl(href), key: href }, text),
      )
      continue
    }
    ...
    if ((cap = this.rules.em.exec(src))) {
      src = src.substring(cap[0].length)
      out.push(
        createElement("em", { key: src.length }, this.output(cap[2] || cap[1])),
      )
      continue
    }
    ...
    if ((cap = this.rules.del.exec(src))) {
      src = src.substring(cap[0].length)
      out.push(createElement("del", { key: src.length }, this.output(cap[1])))
      continue
    }
