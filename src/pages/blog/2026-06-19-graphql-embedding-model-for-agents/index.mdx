...
Schemas reuse field names everywhere. The same `author`, `state`, `createdAt`, `body`, or `title` tends to appear on many different types, so knowing the field name is rarely enough on its own — you have to know *whose* field it is.

Consider a question like *"What is the bug report called?"* against a schema like this:

```graphql
type Issue       { title: String! }
type PullRequest { title: String! }
type Repository  { title: String! }
```

The right answer is `Issue.title`, but every candidate exposes the same field. A general-purpose embedder, trained mostly on prose, doesn't always resolve that ambiguity well and can rank the wrong owner first. At scale it gets worse: in a production schema, hundreds of types can share a single common field name, and retrieval stops being about picking the right field name and starts being about picking the right *owner type*. That owner-type disambiguation is what general embedders are weakest at, and what an agent needs to be reliable.
...
> *"What is the bug report called?"*

Against the three bare coordinates `Issue.title`, `PullRequest.title`, and `Repository.title`, the un-tuned base model narrowly ranks `PullRequest.title` first. The fine-tuned model ranks the correct coordinate, `Issue.title`, first and opens a clearer margin over the other owners:

| model      | `Issue.title` | strongest incorrect candidate | target margin |
|------------|--------------:|------------------------------:|--------------:|
| base       |         0.407 |         0.414 (`PullRequest`) |        -0.007 |
| fine-tuned |         0.589 |         0.498 (`PullRequest`) |        +0.091 |

The field name carries no signal here, since every candidate is `.title`. The owner type is what carries the signal, and that is what the fine-tune learned to weight. This three-candidate comparison is only an illustrative sanity check; the aggregate benchmark above is the evidence for broader retrieval quality.
...
query = "What is the bug report called?"
coords = [
    "Issue.title",
    "PullRequest.title",
    "Repository.title",
...
The fine-tuned model ranks `Issue.title` first in this three-candidate example; the un-tuned base model narrowly ranks `PullRequest.title` first. For a production index, use the richer SDL or one-line-gloss representations described below rather than bare coordinates alone.
