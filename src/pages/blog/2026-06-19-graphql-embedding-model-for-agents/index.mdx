...
To address this, I trained [`Qwen3-Embedding-0.6B-GraphQL`](https://huggingface.co/xthor/Qwen3-Embedding-0.6B-GraphQL), a fine-tune of [`Qwen3-Embedding-0.6B`](https://huggingface.co/Qwen/Qwen3-Embedding-0.6B) focused on a single task: mapping a natural-language question to the `Type.field` coordinate that answers it, with an emphasis on disambiguating between same-named fields on different owner types. It's still an early prototype, but as far as I can tell it's the first open-source embedding model aimed specifically at this job, and I plan to keep training it on broader schemas and more queries over time.

It's a 0.6B parameter model. It runs on CPU, or comfortably alongside an agent's own model on the same GPU. The weights are published as SentenceTransformers and as GGUF builds for `llama.cpp` and Ollama, under Apache 2.0.
