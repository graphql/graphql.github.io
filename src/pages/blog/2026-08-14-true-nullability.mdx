...
service does not support `onError`, the request property should be ignored,
resulting in the traditional behaviour equivalent to `onError: "PROPAGATE"`,
however some legacy servers reject requests with unknown properties. To solve
this, there is an open proposal called [service
capabilities](https://github.com/graphql/graphql-spec/pull/1208) that enables
services to indicate which features, including `onError`, they support. This
enables clients to auto-discover and depend upon these capabilities without
requiring watercooler-based knowledge transfer between backend and
frontend engineers.
...
But serious applications shouldn't use this. Using `onError: "HALT"` disables
one of the big innovations of GraphQL: partial success. Partial success is the
ability to render as much useful content to the user as possible even when some
of your backend systems might be unavailable or running in a degraded state;
it's an under-appreciated feature of GraphQL that's really powerful for helping
your applications to scale rapidly as your userbase grows.

For serious applications, we need something better...
