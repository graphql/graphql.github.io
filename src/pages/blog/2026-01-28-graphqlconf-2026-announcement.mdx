...
Conference registration will be essentially free for attendees.  At the same time, we know that no-shows can make planning difficult and prevent others from attending.  We’re currently working on a ticketing model that encourages attendance while keeping the event accessible.  More details will be shared once the model is finalized.  Our goal is simple: keep GraphQLConf open to everyone, try to offset some expenditures, and respect the community’s time and the generosity of our hosts.
