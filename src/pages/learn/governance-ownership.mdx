...
This guide presents three governance models — centralized, federated, and hybrid — that you can adapt to your organization's structure. Choose based on how much central oversight you want versus team autonomy and iteration speed.
...
This example validates proposed schema changes before the central team reviews them. The automated checks catch GraphQL validity issues, breaking changes, and naming standard violations. For more comprehensive validation, consider [GraphQL Inspector](https://the-guild.dev/graphql/inspector) which provides breaking change detection, schema diffing, and CI integrations.

<Callout type="info">

See the [tools and libraries](/community/tools-and-libraries/) page for available schema validation and linting tools.
...
export function getSubgraphOwner(subgraphName) {
  return subgraphOwnership[subgraphName] || null;
}
```

This example maintains a registry that maps subgraphs to the teams responsible for them. Automation uses this registry to route notifications, assign reviewers, and identify who to contact about specific parts of the schema.
...
When multiple subgraphs contribute fields to the same entity, you can define custom directives to mark ownership. The `@key` directive below comes from [federation specifications](/learn/federation/), while `@owner` is a custom directive you define for your own tooling:
...
This example marks which team owns each portion of an entity. The catalog team owns the base Product definition while inventory owns stock-related extensions. Build tooling that parses the `@owner` directive to route documentation questions and assign reviewers.
...
Validate that subgraph changes compose correctly with all other subgraphs before deployment. The following pseudocode illustrates the general pattern — your schema registry or composition tool handles the specifics:

```javascript
// Pseudocode — use your schema registry's API
...
This example checks composition before publishing a subgraph schema. The validation prevents deployments that would break the composed graph. For open source composition validation, consider [`@graphql-hive/federation-composition`](https://github.com/graphql-hive/federation-composition).
...
See the [federation resources](/resources/federation/) page for composition tools and federation frameworks.
...
For monitoring tools that help track schema usage and breaking changes, see the [monitoring resources](/resources/monitoring/) page.
